...
### 8.6 Seventh pass: the three items left open

| Problem | Fix | Verification |
|---|---|---|
| **The scatter said nothing.** 116 of 118 projects sat on the origin beside two running to thousands. A log scale was built in §8.4 but never engaged, because baseline and forecast each hold genuine zeros (six and four) and a log axis drops them silently | A log scale engages when the *positive* values span orders of magnitude and the records it cannot place are a small minority — and the chart then says how many are missing. Disclosure, not omission: a large share of zeros still forces a linear axis | On the six workbooks the scatter now reads "…tends to rise together. 6 entries with nothing recorded are left out, so the chart can show the very small and the very large together." (§8.8 replaced the original wording, which explained the omission by saying "the scale is logarithmic".) `test_scatter_uses_a_log_scale_and_says_what_it_could_not_plot`, `test_scatter_keeps_every_point_when_the_scale_stays_linear` |
| **One analysis nearly exceeded the rate limit.** OpenAI counted 30,196 tokens against a 30,000 tokens-per-minute ceiling, which is why 429s appear in the worker log. The profile JSON was half of it, and a third of *that* was chart data: every plotted row was shipped inside each chart's visual spec, to a model choosing a layout | The spec is sent without its rows (the row count is kept). `semantic_display` is no longer sent at all — every label it held is already on each entry in `columns`, and nothing reads it from the profile | Profile JSON falls from 42,983 to 33,825 characters, ~10,745 → ~8,456 tokens (−21%), putting a request near 27,900 and under the ceiling. `test_data_profile.py` |
| **"(m)" mid-sentence.** Descriptions lowercased the whole label, flattening units and acronyms | `sentence_label` lowercases words but keeps case that carries meaning — "(M)", "IPA", "%" | `test_sentence_label_keeps_units_and_acronyms_readable` |

**And the weight itself.** §8.4 weighted a percentage by the largest money column, which paired the *financial-year* variance with *whole-life* cost — ten times larger, and measuring something that can span decades rather than the year the variance describes. The weight is now chosen from the percentage's own family: money columns sharing a leading run of at least two words, largest of those. On the six workbooks the headline moves again, from −5.63% to **−11.53%** (the unweighted mean is −4.41%), weighted by financial-year baseline instead of whole-life cost. `test_a_percentage_is_weighted_by_money_from_its_own_family`.

The rate-limit work is worth separating from cost: at $2.50 per 1M input and $10.00 per 1M output tokens, an analysis costs about 8 cents either way. The 21% cut matters because it stops a single request colliding with the per-minute ceiling — a reliability fix that happens to save money, not the other way round.

### 8.8 Eighth pass: can a non-technical reader actually read it?

Asked plainly — "can a business owner understand this without technical terms?" — the answer was no, and the evidence was on the rendered page. The numbers and structure were right; the vocabulary was an analyst's.

Found on screen: "because the scale is **logarithmic**" (added by the previous pass, while calling itself a readability fix), "**Standout records** in 5 **measures**", "**Distribution** and spread of values", "Address the **outliers**", "1 issue detected **before AI reasoning**, plus 4 **notes on spread**", "values fall far outside the **typical range**", "0 numeric values **capped**, 0 **outlier rows** excluded", "118 **rows** · 20 **columns**", and — left in a template — a recommendation offering that a project's variance might be "a one-off **match moment**", football copy in a Treasury portfolio.

| Problem | Fix | Verification |
|---|---|---|
| Analyst vocabulary throughout the deterministic text | Rewritten in ordinary words: "Unusually high or low X", "How many fall into each X", "figures sit well above the rest", "entries"/"fields" rather than rows/columns. Chart titles are rebuilt by the display layer, so "Distribution" had to be fixed there too, not only in the builders | A sweep of all 35 reader-facing strings produced by the pipeline returns no jargon |
| **The model ignored the prompt.** Rule 8 was given an explicit list of words never to use. A *successful* response still wrote: "significant financial variances and **outliers** in the **dataset** … These **anomalies** suggest areas for further investigation" | `analytics/plain_language.py` rewrites model-written text after parsing, at the single point both AI paths return through. Asking was not enough; the wording is now enforced | `test_plain_language.py`, whose fixtures are the model's verbatim output rather than invented examples |

Three defects of my own, all caught by looking rather than assuming:

- The plain-language filter **did nothing to a sentence that opened with the jargon** — `re.sub` without `IGNORECASE`, so "Outliers dominate…" passed straight through while the same word mid-sentence was replaced. The paragraph-level test passed and gave false reassurance; only the boundary case exposed it.
...
**The filter also produced its own clumsy English — twice.** It first swapped "outliers" for the phrase "unusually high or low figures", which doubled a noun ("some financial figures are unusually high or low **figures**"). A special-case rule collapsed that one sentence shape, and the next render showed another: "shows notable financial **unusually high or low figures**" — a four-word phrase dropped in after adjectives. Patching sentence shapes one at a time was the wrong approach. "Outliers" now becomes "extremes", a single noun that reads correctly wherever the original word sat ("notable financial extremes", "extremes in the file", "Address the extremes"), and the special case is gone. A replacement table that makes text awkward is only a better class of unreadable.
